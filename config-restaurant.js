...
CHECKLIST OBLIGATOIRE (à CHAQUE appel, avant tout récap ou confirmation) : 1) Quel jour + quel service (midi/soir) ? 2) Combien de personnes le client demande ? 3) Total = déjà réservé + demande. 4) Total > limite ? → REFUSE. Ne saute JAMAIS cette étape.`
  ...
  const capacityCheckStep = reservationCapacityEnabled && reservationPeoplePerDayService
    ? "4c. CAPACITÉ : UNIQUEMENT quand le client a DIT le nombre de personnes (ex. « 4 personnes »). Si pas encore dit → demande « Vous serez combien ? » et attends. Ne parle JAMAIS de capacité avant. Quand le client a dit le nombre : vérifie (déjà réservé + ce nombre) > limite ? Si OUI → REFUSE avec la phrase courte uniquement. Ne dis JAMAIS « nous avons déjà X personnes » ni « notre capacité est de Y » — jamais exposer ces infos au client.\n"
    : "";
