...
NE dis JAMAIS dans ce cas « on ne prend plus de réservations après 21h » ni « après l'heure limite » — c'est COMPLET (plus de place), pas une question d'heure. Si le client accepte un autre jour, alors seulement tu enchaînes avec les questions (midi ou soir, heure, etc.).`
  ...
  const terrasseRule = hasTerrace
    ? "- Terrasse/intérieur — NE JAMAIS INVERSER : TERRASSE = dehors, extérieur, en plein air. INTÉRIEUR = dedans, à l'intérieur. Si le client dit \"terrasse\", \"en terrasse\", \"sur la terrasse\", \"dehors\" → tu notes TERRASSE (pas intérieur). Si le client dit \"intérieur\", \"à l'intérieur\", \"dedans\", \"en salle\" → tu notes INTÉRIEUR (pas terrasse). Vérifie le mot exact : un client qui dit \"terrasse\" veut la TERRASSE ; un client qui dit \"intérieur\" veut l'INTÉRIEUR. En cas de doute (audio peu clair), demande : \"Terrasse ou intérieur ?\" pour confirmer. Si le client ne l'a pas dit, tu DOIS demander \"Terrasse ou intérieur ?\". Ne passe jamais au récap sans cette préférence."
...
- TERRASSE / INTÉRIEUR — NE PAS INVERSER : Quand le client répond à \"Terrasse ou intérieur ?\", note EXACTEMENT ce qu'il dit. \"Terrasse\" ou \"en terrasse\" → TERRASSE (dehors). \"Intérieur\" ou \"à l'intérieur\" → INTÉRIEUR (dedans). Ne note JAMAIS terrasse si le client a dit intérieur, ni intérieur si le client a dit terrasse.
