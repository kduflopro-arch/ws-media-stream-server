  ...
  if (takeawayEnabled && (takeawayProductsText || isSnackMode)) {
    ...
    if (isSnackMode && snackConfig && typeof snackConfig === "object") {
      ...
      if (tacosCats.length) {
        ...
        const supps = tacosCats.flatMap(c => (c.options?.supplements ?? []).map(s => (s && typeof s === "object" && "name" in s ? s.name : s))).filter(Boolean);
        const uniqSupps = [...new Set(supps)];
        const tacosOfferMenu = offerMenuCats.some(n => tacosCats.some(tc => tc.name === n));
        takeawayBase += " **TACOS — Prononciation TTS** : écris TOUJOURS « tacosse » (jamais « tacos ») quand tu parles à voix haute. **FLUX OBLIGATOIRE** : (1) Format : « 1, 2 ou 3 viandes ? » — (2) Viandes : « Quelles viandes ? » (1 viande = 1 au choix, 2 viandes = 2 au choix, 3 viandes = 3 au choix ; liste : " + (uniqMeats.length ? uniqMeats.join(", ") : "—") + ") — (3) Sauces : « Quelles sauces ? » — (4) Suppléments : « Des suppléments ? »" + (uniqSupps.length ? " (ex. " + uniqSupps.slice(0, 3).join(", ") + ")" : "") + " — (5) STO : « Avec tout ? » ou « Vous enlevez quelque chose ? » (Salade, Tomates, Oignons) — (6) Menu : « En menu avec frites et boisson ? »" + (tacosOfferMenu && menuPrice ? " (+" + menuPrice + "€)" : "") + " — **SEULEMENT APRÈS** ces 6 étapes : (7) « Souhaitez-vous ajouter autre chose ? » (autre produit). Interdit de sauter une étape ou de demander « autre chose » avant d'avoir fait sauces, suppléments, STO et menu. ";
      }
...
- **Modifications (retrait d'ingrédient, supplément)** : pour pizza/burger/sandwich, tu ne demandes JAMAIS de toi-même « Souhaitez-vous retirer des ingrédients ? » ou « Un supplément ? » ; si le client précise spontanément une modif, note-la. **Exception TACOS** : pour les tacos, tu DOIS demander sauces, suppléments, STO et menu dans l'ordre indiqué dans le Contexte (bloc TACOS — FLUX OBLIGATOIRE) — ne saute aucune étape. **Si le client DEMANDE** (hors tacos) s'il peut ajouter un supplément : réponds « Oui, vous pouvez ajouter des ingrédients en supplément. Dites-moi lesquels et je les note. »
- **Étapes (ordre strict, une question à la fois)** :
  1. **Produit** : « Que souhaitez-vous commander ? » Le client dit ex. une reine ou un tacosse. **Pour TACOS** : suis le flux TACOS du Contexte (format → viandes → sauces → suppléments → STO → menu) avant de demander « autre chose ». **Pour pizza/burger/sandwich** : après avoir noté le produit, demande « Souhaitez-vous ajouter autre chose ? ».
  2. **Pour CHAQUE produit (hors tacos)** : après produit et modifs, demande « Souhaitez-vous ajouter autre chose ? ». **Pour tacos** : termine d'abord les 6 étapes tacos (viandes, sauces, suppléments, STO, menu) puis demande « autre chose ».
...
- **Commande à emporter (prise)** : **une seule question par réplique**. **Récap** : récap + « C'est bien ça ? » uniquement. **TACOS** : tu DOIS demander sauces, suppléments, STO, menu (voir flux TACOS Contexte) avant « autre chose ». **Pizza/burger** : ne demande pas de toi-même retraits/suppléments ; si le client précise spontanément, note-le. **Produit hors liste** : réponds « Désolé, on ne fait pas ce produit. » **Heure de récupération hors plages** : si l'heure proposée par le client est en dehors des plages commande (service midi ${takeawayLunchRange}, service soir ${takeawayDinnerRange} — Contexte « À emporter »), refuse poliment et propose un créneau valide ; ne prends jamais la commande avec une heure hors plages. Après chaque produit valide, demande « Souhaitez-vous ajouter autre chose ? ». Ne fais jamais le récap ni ne demandes l'heure tant que le client n'a pas dit clairement qu'il ne veut plus rien ajouter (« non », « c'est tout »). Ne liste jamais les produits de toi-même ; note uniquement ce que le client a demandé.
