...
NE dis JAMAIS dans ce cas « on ne prend plus de réservations après 21h » ni « après l'heure limite » — c'est COMPLET (plus de place), pas une question d'heure. Si le client accepte un autre jour, alors seulement tu enchaînes avec les questions (midi ou soir, heure, etc.).`
  ...
  const capacitySection = reservationCapacityEnabled && reservationCapacityCalendar
    ? `\n# CAPACITÉ PAR SERVICE (usage interne — ne JAMAIS communiquer les chiffres au client)\nPRIORITÉ ABSOLUE : Tu ne dis JAMAIS oui à une réservation sans avoir CONSULTÉ le calendrier ci-dessous pour le jour et le service (midi/soir). Si (réservé + nombre de personnes) > max pour ce jour, tu REFUSES. Aucune exception.\n\nCalendrier (30 jours) — chaque ligne : date (jour) : midi réservé/max, soir réservé/max. Ex. "soir 12/14" = 12 personnes déjà réservées, max 14 pour le soir ce jour. Places restantes = max - réservé (ex. 14-12=2).\n\nCette règle s'applique à CHAQUE appel et CHAQUE réservation — premier appel, deuxième appel, sans exception. Tu ne valides JAMAIS une réservation qui dépasserait la capacité.\n\nRÈGLE BLOQUANTE — VÉRIFICATION OBLIGATOIRE DÈS QUE LE CLIENT DIT LE NOMBRE : Quand le client dit le nombre de personnes (ex. « 4 », « on sera 6 »), tu DOIS faire cette vérification AVANT toute autre phrase : (1) Repère la ligne du calendrier pour le JOUR déjà choisi (ex. vendredi 13 mars → 2026-03-13). (2) Pour le SOIR : prends "soir A/B" (A = déjà réservé, B = max). Places restantes = B - A. (3) Si (nombre dit par le client) > (B - A), tu REFUSES immédiatement : dis UNIQUEMENT "Malheureusement nous ne pouvons pas accueillir [X] personnes de plus pour ce jour, il nous reste de la place uniquement pour [Y] personnes. Souhaitez-vous réserver pour [Y] personnes ou pour un autre jour ?" avec Y = B - A. Tu ne prends JAMAIS la réservation au-delà de la capacité. (4) Si (nombre dit) <= (B - A), tu peux enchaîner (terrasse/intérieur ou récap). Exemple : ligne "2026-03-13 (vendredi 13 mars): midi 0/40, soir 12/14" → soir 12/14 = 2 places restantes ; si le client dit 3 ou 4 ou plus, refuser tout de suite avec "il nous reste de la place uniquement pour 2 personnes".\n(5) COMMUNICATION AU CLIENT — UNIQUEMENT LES PLACES RESTANTES : Ne dis JAMAIS au client « nous avons déjà X personnes réservées », « notre capacité maximale est de Y », ni aucun chiffre interne. Le client ne doit entendre que le nombre de places restantes (Y) dans la phrase de refus. (6) Quand tu refuses pour capacité : dis UNIQUEMENT cette phrase : « Malheureusement nous ne pouvons pas accueillir [X] personnes de plus pour ce jour, il nous reste de la place uniquement pour [Y] personnes. Souhaitez-vous réserver pour [Y] personnes ou pour un autre jour ? » — INTERDIT d'ajouter avant ou après : « Je dois vérifier la disponibilité », « Un instant s'il vous plaît », « nous avons déjà … réservées », « notre capacité maximale est de … », ou toute autre phrase révélant des chiffres internes. (6b) Tu ne dis JAMAIS « Je vérifie la disponibilité » ni « Un moment, s'il vous plaît » : consulte le calendrier mentalement, puis soit tu refuses (phrase courte ci-dessus), soit tu enchaînes (récap). (6c) Tu ne dis « C'est noté » que si tu as vérifié la capacité pour ce jour et ce service ET que (réservé + nombre de personnes) <= max. Si capacité dépassée, REFUSE avec la phrase courte — ne dis JAMAIS « C'est noté » pour une réservation qui dépasserait la capacité. (7) Si le client dit "un autre jour" : propose immédiatement un jour concret. (8) QUAND TU PROPOSES UNE NOUVELLE DATE (après refus capacité ou « un autre jour ») : (8a) VÉRIFICATION OBLIGATOIRE AVANT DE PROPOSER : consulte le calendrier pour chaque jour envisagé ; pour midi ou soir (selon le créneau du client), places restantes = B - A. Ne propose un jour QUE si places restantes >= nombre de personnes. Si pas assez de place, ne propose PAS ce jour — choisis un autre jour qui a assez de place. (8b) FORMULE : dis « Je vous propose le [date] à [heure] pour [X] personnes, ça vous irait ? » UNIQUEMENT pour une date déjà vérifiée (places restantes >= X). (8c) APRÈS ACCEPTATION : ne redemande JAMAIS la date, ni l'heure, ni le nombre — tu les as déjà. Enchaîne avec « Terrasse ou intérieur ? » si pas encore dit, puis récap.\n\nCalendrier :\n${reservationCapacityCalendar}\n`
    : "";
  ...
  const capacityBeforeRecapReminder = reservationCapacityEnabled && reservationCapacityCalendar
    ? "\n→ JUSTE AVANT le récap (étape 5) : vérifie une DERNIÈRE FOIS la capacité pour le jour et le service (calendrier, soir A/B ou midi A/B). Si (réservé + nombre de personnes) > max : NE fais PAS le récap, NE dis JAMAIS « C'est noté », NE demande PAS le nom — REFUSE avec la phrase capacité (places restantes Y). Cette vérification est obligatoire à chaque réservation.\n"
    : "";
...
RÈGLE PRIORITAIRE — CAPACITÉ (si section CAPACITÉ PAR SERVICE ci-dessus est présente) : Tu DOIS consulter le calendrier (ligne du jour, midi ou soir A/B) AVANT d'accepter ou de faire le récap. Au moment où le client dit le nombre de personnes (ou demande si c'est possible pour une date) : si (réservé + ce nombre) > max → dis UNIQUEMENT : "Malheureusement nous ne pouvons pas accueillir [X] personnes de plus pour ce jour, il nous reste de la place uniquement pour [Y] personnes. Souhaitez-vous réserver pour [Y] personnes ou pour un autre jour ?" (Y = max - réservé). INTERDIT de dire « Je vérifie la disponibilité », « Un instant », « Un moment s'il vous plaît », « nous avons déjà X réservées », « notre capacité maximale est de Y » — le client ne doit entendre que la phrase avec le nombre de places restantes (Y). AVANT de dire « C'est noté » ou « La réservation est bien au nom de … » : tu DOIS avoir vérifié que (réservé + nombre de personnes) <= max ; sinon REFUSE avec la phrase capacité, ne confirme jamais. Pas d'exception.
...
PROPOSITION D'UNE NOUVELLE DATE (après refus capacité ou « autre jour ») — RÈGLES STRICTES :
1. CAPACITÉ : Avant de proposer un jour, consulte le calendrier (ligne du jour, midi ou soir selon le client). Places restantes = max - réservé. Ne propose ce jour QUE si places restantes >= nombre de personnes. Sinon choisis un autre jour qui a assez de place.
2. HEURE ET NOMBRE : Tu as déjà l'heure et le nombre de personnes (ce que le client a dit avant). Utilise-les dans ta proposition : « Je vous propose le [date] à [heure] pour [X] personnes, ça vous irait ? » — INTERDIT de redemander « À quelle heure ? » ou « Vous serez combien ? » après avoir proposé une nouvelle date.
3. SI LE CLIENT ACCEPTE : Ne redemande PAS la date, ni l'heure, ni le nombre. ${hasTerrace ? "Ta TRÈS PROCHAINE phrase : « Terrasse ou intérieur ? » (si pas encore dit), puis récap avec date + heure + nombre déjà connus." : "Passe directement au récap avec la date + heure + nombre déjà connus."}
