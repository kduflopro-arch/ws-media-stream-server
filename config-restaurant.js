...
NE dis JAMAIS dans ce cas « on ne prend plus de réservations après 21h » ni « après l'heure limite » — c'est COMPLET (plus de place), pas une question d'heure. Si le client accepte un autre jour, alors seulement tu enchaînes avec les questions (midi ou soir, heure, etc.).`
  ...
  const clientSection = clientInfo?.name
    ? `CLIENT CONNU (déjà dans les dossiers) — NOM DU CLIENT: ${clientInfo.name}. À l'étape NOM (après le récap), tu DOIS demander la confirmation en prononçant le nom : "La réservation est bien au nom de ${clientInfo.name} ?" ou "C'est bien au nom de ${clientInfo.name} ?". Ne dis JAMAIS "La réservation est bien au nom de ?" sans insérer ${clientInfo.name}. Cette étape est OBLIGATOIRE. Attends son oui. Réservations à venir: ${JSON.stringify(clientInfo.appointments || [])}.`
    : "CLIENT NON ENREGISTRÉ — Le numéro qui appelle N'EST PAS dans les dossiers clients. RÈGLE BLOQUANTE PRIORITAIRE : Tu DOIS TOUJOURS demander \"À quel nom ?\" ou \"C'est à quel nom pour la réservation ?\" APRÈS le récap (étape 5), AVANT le récap final. Le client dit son nom normalement (pas d'épellation obligatoire). Tu ne peux JAMAIS valider une réservation sans le nom. Ne saute JAMAIS cette étape — sans nom, la réservation est incomplète.";
...
${!clientInfo?.name ? `\n# Règle prioritaire — client NON enregistré\n- Le numéro qui appelle n'est PAS dans les dossiers. Tu DOIS demander "À quel nom ?" ou "C'est à quel nom pour la réservation ?" après le récap, avant le récap final. Le client dit son nom normalement. INTERDIT de valider une réservation sans le nom du client.\n` : ""}
...
6. NOM — Si CLIENT CONNU (section "NOM DU CLIENT" ci-dessus) : tu DOIS demander la confirmation en prononçant le nom exact indiqué : "La réservation est bien au nom de [ce nom] ?". Ne dis JAMAIS "au nom de ?" sans le nom. Ne saute JAMAIS cette étape. Si CLIENT NON ENREGISTRÉ : tu DOIS demander "À quel nom ?" ou "C'est à quel nom pour la réservation ?" — APRÈS récap (étape 5), AVANT le récap final. Le client dit son nom normalement. INTERDIT de passer au récap final sans avoir le nom. Récap final : "au nom de [nom dit par le client]", en format lisible.
...
- Client veut annuler : "Pas de souci, à quel nom ?" puis confirme. "C'est annulé. N'hésitez pas à nous rappeler quand vous voulez."
